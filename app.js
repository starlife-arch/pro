...
function wireForm() {
  document.getElementById("assetImagesInput").addEventListener("change", handleAssetImageSelect);
...
function renderMyAssets() {
  ...
  grid.innerHTML = myAssets
    .map((ua) => {
      ...
      return `
      <article class="card">
        ${asset.images?.[0] ? `<img class="asset-thumb" src="${asset.images[0]}" alt="${asset.name}" />` : ""}
        <h3>${asset.name}</h3>
        <div class="tags">
...
function assetCard(asset) {
  ...
  return `<article class="card">
    ${image ? `<img class="asset-thumb" src="${image}" alt="${asset.name}" />` : ""}
    <h3>${asset.name}</h3>
    <div class="tags">
...
function openAssetDetails(assetId) {
  ...
  const gallery = (asset.images || []).length
    ? `<div class="asset-gallery">${asset.images.map((img) => `<img src="${img}" alt="${asset.name}" />`).join("")}</div>`
...
function handleAssetImageSelect(event) {
  ...
  if (pendingAssetImages.length >= 3) {
    toast("You can upload up to 3 images only.");
    event.target.value = "";
    return;
  }

  const allowedSlots = 3 - pendingAssetImages.length;
  const selectedFiles = files.slice(0, allowedSlots);

  Promise.all(selectedFiles.map(readAssetImageFile))
    .then((images) => {
      pendingAssetImages.push(...images);
      renderAssetImagePreview();
      if (files.length > allowedSlots) toast("Only first 3 images were kept.");
    })
    .catch((error) => toast(error.message || "Could not process selected image."))
    .finally(() => {
      event.target.value = "";
    });
}

function readAssetImageFile(file) {
  return new Promise((resolve, reject) => {
    if (!file.type.startsWith("image/")) {
      reject(new Error("Only image MIME types are allowed."));
      return;
    }
    if (file.size > 1500 * 1024) {
      reject(new Error("Image too large. Max 1.5MB before compression."));
      return;
    }
    compressImg(file, (b64) => resolve(b64));
  });
}

function renderAssetImagePreview() {
  const previewWrap = document.getElementById("assetImagePreview");
  if (!previewWrap) return;
  if (!pendingAssetImages.length) {
    previewWrap.innerHTML = "<small>No images selected.</small>";
    return;
  }

  previewWrap.innerHTML = pendingAssetImages
    .map((image, index) => `<div class="image-preview-item"><img src="${image}" alt="asset image ${index + 1}" /><button type="button" data-remove-image="${index}">✕</button></div>`)
    .join("");

  previewWrap.querySelectorAll("[data-remove-image]").forEach((button) => {
    button.addEventListener("click", () => {
      const index = Number(button.dataset.removeImage);
      pendingAssetImages.splice(index, 1);
      renderAssetImagePreview();
    });
  });
}
